...
export const Loader: FC<StandaloneLoaderProps> = memo(
  ({ message, size = 40, bgOpacity = 50, isVisible = false }) => {
    ...
    return (
      ...
      >
        <div
          className={`max-w-sm flex flex-col items-center transform transition-transform duration-300 ease-in-out`}
        >
          <Loader2
            size={size}
            aria-hidden="true"
            className="animate-spin text-teal mb-4"
          />

          {message && (
            <p
              id="standalone-loading-title"
