...
import { useCommitteeStaffsDrawer, useTerritorialStaffsDrawer } from '@/shared/hooks/entity-hooks';
...
export const useStaffs = () => {
  ...
  const { onOpen: openCommitteeStaffsDrawer, isOpen: isOpenCommitteeStaffs } = useCommitteeStaffsDrawer();

  const { onOpen: openTerritorialStaffsDrawer, isOpen: isOpenTerritorialStaffs } = useTerritorialStaffsDrawer();

  const activeTab = useMemo<StaffsActiveTab>(() => filters['active-tab'] as StaffsActiveTab, [filters]);

  const handleChangeTab = useCallback(
    (tab: StaffsActiveTab) => setFilters((prev: any) => ({ ...prev, 'active-tab': tab })),
    [setFilters],
  );
