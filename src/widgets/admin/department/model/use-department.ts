...
export const useDepartment = () => {
  ...
  const { onOpen: openCentralApparatusDrawer, isOpen: isOpenCentralApparatus } =
    useCentralApparatusDrawer();

  const {
    onOpen: openTerritorialDepartmentsDrawer,
    isOpen: isOpenTerritorialDepartments,
  } = useTerritorialDepartmentsDrawer();

  const activeTab = useMemo<DepartmentActiveTab>(
    () => filters['active-tab'] as DepartmentActiveTab,
    [filters],
  );

  const handleChangeTab = useCallback(
    (tab: DepartmentActiveTab) =>
