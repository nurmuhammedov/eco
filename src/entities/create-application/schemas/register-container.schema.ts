...
export const ContainerAppealDtoSchema = z.object({
  ...
  partialCheckDate: z.date({ required_error: 'Majburiy maydon!' }).transform((date) => format(date, 'yyyy-MM-dd')), // Tashqi va ichki ko‘rik
  fullCheckDate: z.date({ required_error: 'Majburiy maydon!' }).transform((date) => format(date, 'yyyy-MM-dd')), // Gidrosinov
