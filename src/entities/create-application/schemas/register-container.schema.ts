...
export const ContainerAppealDtoSchema = z.object({
  phoneNumber: z
  ...
  nonDestructiveCheckDate: z // Putur yetkazmaydigan nazoratdan o'tkazish sanasi
    ...
    .transform((date) => format(date, 'yyyy-MM-dd')),
  capacity: z.string({ required_error: 'Hajmi kiritilmadi!' }).min(1, 'Hajmi kiritilmadi!'), // Hajmi
  environment: z.string({ required_error: 'Muhit kiritilmadi!' }).min(1, 'Muhit kiritilmadi!'), // Muhit
  pressure: z
    .string({ required_error: 'Ruxsat etilgan bosim kiritilmadi!' })
    .min(1, 'Ruxsat etilgan bosim kiritilmadi!'), // Ruxsat etilgan bosim
});
