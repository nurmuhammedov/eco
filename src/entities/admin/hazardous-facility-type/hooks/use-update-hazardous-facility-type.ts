...
export const useUpdateHazardousFacilityType = () => {
  ...
  return useMutation({
    ...
    onMutate: async (updatingData: UpdateHazardousFacilityTypeDTO) => {
      ...
      const previousRegionDetail =
        queryClient.getQueryData<UpdateHazardousFacilityTypeDTO>(
      ...
      const previousListData = queryClient.getQueryData<
        ResponseData<UpdateHazardousFacilityTypeDTO>
      ...
      if (previousListData) {
        queryClient.setQueryData(
          ...
          {
            ...previousListData,
            content: previousListData.content.map((item) =>
              item.id === updatingData.id ? updatingData : item,
            ),
          },
        );
      }
