...
import { updateDirection } from '@/entities/attestation/api/attestation.api'
import { directionSchema } from '@/entities/attestation/model/schema'
import { AttestationDirection, UpdateDirectionPayload } from '@/entities/attestation/model/types'
...
interface EditDirectionModalProps {
  isOpen: boolean
  onClose: () => void
  direction: AttestationDirection | null
}

export const EditDirectionModal: FC<EditDirectionModalProps> = ({ isOpen, onClose, direction }) => {
  const queryClient = useQueryClient()
  const form = useForm<UpdateDirectionPayload>({
  ...
  const onSubmit = (data: UpdateDirectionPayload) => {
    if (!direction) return
    mutate({ id: direction.id, data })
  }
