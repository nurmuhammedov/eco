...
export const useRegisterIllegalPipeline = (externalSubmit?: (data: RegisterIllegalPipelineDTO) => void) => {
  ...
  const form = useForm<RegisterIllegalPipelineDTO>({
  ...
  const { mutateAsync: updateMutate, isPending: isUpdatePending } = useUpdate('/equipments/pipeline/', id)
  ...
  useEffect(() => {
    if (detail && isUpdate) {
      ...
      form.reset({
        ...
        diameter: getValue(detail.parameters?.diameter || ''),
        thickness: getValue(detail.parameters?.thickness || ''),
  ...
  const handleSubmit = (data: RegisterIllegalPipelineDTO) => {
    if (isUpdate) {
      const updatePayload = {
        ...data,
