...
export const useRegisterIllegalContainer = (externalSubmit?: (data: RegisterIllegalContainerDTO) => void) => {
  ...
  const form = useForm<RegisterIllegalContainerDTO>({
  ...
  const handleSubmit = (data: RegisterIllegalContainerDTO) => {
    if (isUpdate) {
      const updatePayload = {
        ...data,
