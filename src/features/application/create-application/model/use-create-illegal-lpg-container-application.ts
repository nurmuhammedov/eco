import {
  lpgContainerRefinement,
  RegisterIllegalLpgContainerBaseSchema,
  RegisterIllegalLpgContainerDTO,
...
export const useRegisterIllegalLpgContainer = (externalSubmit?: (data: RegisterIllegalLpgContainerDTO) => void) => {
  ...
  const form = useForm<RegisterIllegalLpgContainerDTO>({
  ...
  const { mutateAsync: updateMutate, isPending: isUpdatePending } = useUpdate('/equipments/lpg-container/', id)
  ...
  const handleSubmit = (data: RegisterIllegalLpgContainerDTO) => {
    if (isUpdate) {
      const updatePayload = {
        ...data,
