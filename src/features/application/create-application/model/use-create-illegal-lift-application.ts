...
import { useDetail, useTranslatedObject, useUpdate } from '@/shared/hooks'
...
import {
  liftRefinement,
  RegisterIllegalLiftBaseSchema,
  RegisterIllegalLiftDTO,
  RegisterIllegalLiftSchema,
} from '@/entities/create-application/schemas/register-illegal-lift.schema'

export const useRegisterIllegalLift = (externalSubmit?: (data: RegisterIllegalLiftDTO) => void) => {
  ...
  const form = useForm<RegisterIllegalLiftDTO>({
  ...
  useEffect(() => {
    if (detail && isUpdate) {
      ...
      form.reset({
        ...
        stopCount: getValue(detail.parameters?.stopCount || ''),
        sphere: detail?.sphere || undefined,
  ...
  const handleSubmit = (data: RegisterIllegalLiftDTO) => {
    if (isUpdate) {
      const updatePayload = {
        ...data,
