import {
  chemicalContainerRefinement,
  RegisterIllegalChemicalContainerBaseSchema,
  RegisterIllegalChemicalContainerDTO,
...
export const useRegisterIllegalChemicalContainer = (
  ...
  const form = useForm<RegisterIllegalChemicalContainerDTO>({
  ...
  const { mutateAsync: updateMutate, isPending: isUpdatePending } = useUpdate('/equipments/chemical-container/', id)
  ...
  const handleSubmit = (data: RegisterIllegalChemicalContainerDTO) => {
    if (isUpdate) {
      const updatePayload = {
        ...data,
