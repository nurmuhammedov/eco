...
import { useXrayDetail } from '@/features/register/xray/hooks/use-xray-detail'
...
const XrayDetail = () => {
  const { id } = useParams()
  const { isLoading, data, refetch } = useXrayDetail()
  const { user } = useAuth()
  const [isDeregisterModalOpen, setIsDeregisterModalOpen] = useState(false)
  const canDeregister = user?.role === UserRoles.MANAGER && data?.isActive
