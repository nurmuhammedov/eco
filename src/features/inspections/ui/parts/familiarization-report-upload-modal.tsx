...
const FamiliarizationReportUploadModal = ({ resultId, familiarizationReportPath, onClose, trigger }: Props) => {
  const { t } = useTranslation()
  const [open, setOpen] = useState(false)
  const qc = useQueryClient()
  const { mutateAsync: uploadAct, isPending } = useAdd(
    `/inspection-results/${resultId}/familiarization-report`,
    'Tushuntirish bayonnomasi muvaffaqiyatli saqlandi'
  )
  ...
  useEffect(() => {
    if (open) {
      form.reset({ familiarizationReportPath: familiarizationReportPath || '' })
    }
  }, [open, familiarizationReportPath, form])
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="mt-2 space-y-6">
            <FormField
              ...
              name="familiarizationReportPath"
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="mb-1" required={true}>
                    Tushuntirish bayonnomasi
                  </FormLabel>
                  <InputFile
...
export default FamiliarizationReportUploadModal
