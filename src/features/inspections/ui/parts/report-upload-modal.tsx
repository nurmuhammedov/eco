...
const ReportUploadModal = ({ resultId, reportPath, onClose, trigger }: Props) => {
  const { t } = useTranslation()
  const [open, setOpen] = useState(false)
  const qc = useQueryClient()
  const { mutateAsync: uploadAct, isPending } = useAdd(
    `/inspection-results/${resultId}/report`,
    'Bayonnoma muvaffaqiyatli saqlandi'
  )
  ...
  useEffect(() => {
    if (open) {
      form.reset({ reportPath: reportPath || '' })
    }
  }, [open, reportPath, form])
